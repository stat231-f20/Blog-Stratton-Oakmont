...
gf_point(log(revenue_growth) ~ sqrt(esg), data = esg_grouped)
...
# none of the transformations are very close to normal, but log(esg) seems to remove most of the skew
gf_histogram(~esg, data = esg_grouped)
gf_histogram(~ log(esg), data = esg_grouped)
gf_histogram(~ sqrt(esg), data = esg_grouped)

# proceed with log(revenue_growth) and log)esg
...
# multiple regression by sector group 
mm2 <- lmList(log(revenue_growth) ~ log(esg)|sector_group, esg_grouped)
summary(mm2)
...
gf_point(revenue_growth ~ esg, data = esg_grouped)
gf_point(log(revenue_growth) ~ log(esg), data = esg_grouped)
